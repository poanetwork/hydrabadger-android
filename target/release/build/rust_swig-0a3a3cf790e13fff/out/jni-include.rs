...
pub unsafe fn jlong_to_pointer<T>(val: jlong) -> *mut T { val as *mut T }
#[allow(dead_code)]
...
macro_rules! jni_unpack_return((
                               $ result_value : expr , $ func_ret_type : ty ,
                               $ env : ident ) => {
                               {
                               let ret = match $ result_value {
                               Ok ( x ) => x , Err ( msg ) => {
                               jni_throw_exception ( $ env , & msg ) ; return
                               < $ func_ret_type > :: invalid_value (  ) ; } }
                               ; ret } } ;);
...
impl SwigFrom<i8> for jbyte {
    fn swig_from(x: i8, _: *mut JNIEnv) -> Self { x }
}
impl SwigInto<i8> for jbyte {
    fn swig_into(self, _: *mut JNIEnv) -> i8 { self }
}
impl SwigFrom<u8> for jshort {
    fn swig_from(x: u8, _: *mut JNIEnv) -> Self { jshort::from(x) }
}
impl SwigInto<u8> for jshort {
    fn swig_into(self, _: *mut JNIEnv) -> u8 {
        if self < 0 || self > (::std::u8::MAX as jshort) {
            panic!("Expect self from 0 to {}, got {}" , :: std :: u8 :: MAX ,
                   self);
        }
        self as u8
    }
...
impl SwigInto<i16> for jshort {
    fn swig_into(self, _: *mut JNIEnv) -> i16 { self }
}
impl SwigFrom<i16> for jshort {
    fn swig_from(x: i16, _: *mut JNIEnv) -> Self { x }
}
impl SwigFrom<u16> for jint {
    fn swig_from(x: u16, _: *mut JNIEnv) -> Self { jint::from(x) }
}
impl SwigInto<u16> for jint {
    fn swig_into(self, _: *mut JNIEnv) -> u16 {
        if self < 0 || self > (::std::u16::MAX as jint) {
            panic!("Expect self from 0 to {}, got {}" , :: std :: u16 :: MAX ,
                   self);
        }
        self as u16
    }
...
impl SwigFrom<i32> for jint {
    fn swig_from(x: i32, _: *mut JNIEnv) -> Self { x }
}
impl SwigFrom<u32> for jlong {
    fn swig_from(x: u32, _: *mut JNIEnv) -> Self { jlong::from(x) }
}
impl SwigInto<u32> for jlong {
    fn swig_into(self, _: *mut JNIEnv) -> u32 {
        if self < 0 || self > (::std::u32::MAX as jlong) {
            panic!("Expect self from 0 to {}, got {}" , :: std :: u32 :: MAX ,
                   self);
        }
        self as u32
    }
...
impl SwigInto<i64> for jlong {
    fn swig_into(self, _: *mut JNIEnv) -> i64 { self }
}
impl SwigFrom<i64> for jlong {
    fn swig_from(x: i64, _: *mut JNIEnv) -> Self { x }
}
impl SwigInto<u64> for jlong {
    fn swig_into(self, _: *mut JNIEnv) -> u64 {
        if self < 0 { panic!("Expect self to be positive, got {}" , self); }
        self as u64
    }
}
...
impl SwigInto<f32> for jfloat {
    fn swig_into(self, _: *mut JNIEnv) -> f32 { self }
}
impl SwigFrom<f32> for jfloat {
    fn swig_from(x: f32, _: *mut JNIEnv) -> Self { x }
}
impl SwigInto<f64> for jdouble {
    fn swig_into(self, _: *mut JNIEnv) -> f64 { self }
}
impl SwigFrom<f64> for jdouble {
    fn swig_from(x: f64, _: *mut JNIEnv) -> Self { x }
}
...
impl SwigFrom<SystemTime> for jobject {
    fn swig_from(x: SystemTime, env: *mut JNIEnv) -> Self {
        let since_unix_epoch =
            x.duration_since(::std::time::UNIX_EPOCH).unwrap();
        let mills: jlong =
            (since_unix_epoch.as_secs() * 1000 +
                 ((since_unix_epoch.subsec_nanos() / 1000000) as u64)) as
                jlong;
        let date_class: jclass =
            unsafe {
                (**env).FindClass.unwrap()(env, swig_c_str!("java/util/Date"))
            };
        assert!(! date_class . is_null (  ) ,
                "FindClass for `java/util/Date` failed");
        let init: jmethodID =
            unsafe {
                (**env).GetMethodID.unwrap()(env, date_class,
                                             swig_c_str!("<init>"),
                                             swig_c_str!("(J)V"))
            };
        assert!(! init . is_null (  ) ,
        ...
        let x =
            unsafe {
                (**env).NewObject.unwrap()(env, date_class, init, mills)
            };
        assert!(! x . is_null (  ));
        x
    }
...
impl SwigInto<usize> for i64 {
    fn swig_into(self, _: *mut JNIEnv) -> usize {
        if self < 0 {
            panic!("{}:{} expect self to be positive, got {}" , file ! (  ) ,
                   line ! (  ) , self);
        } else if (self as u64) > (::std::usize::MAX as u64) {
            panic!("{}:{} too big value for usize {}" , file ! (  ) , line ! (
                    ) , self);
        } else { self as usize }
    }
}
impl <'a> SwigInto<&'a Path> for &'a str {
    fn swig_into(self, _: *mut JNIEnv) -> &'a Path { Path::new(self) }
}
impl SwigInto<jobjectArray> for Vec<String> {
    fn swig_into(mut self, env: *mut JNIEnv) -> jobjectArray {
        ...
        let obj_arr: jobjectArray =
            unsafe {
                (**env).NewObjectArray.unwrap()(env, self.len() as jsize,
        ...
        for (i, r_str) in self.drain(..).enumerate() {
            ...
            assert!(! jstr . is_null (  ));
            unsafe {
                (**env).SetObjectArrayElement.unwrap()(env, obj_arr,
                                                       i as jsize, jstr);
                if (**env).ExceptionCheck.unwrap()(env) != 0 {
                    panic!("SetObjectArrayElement({}) failed" , i);
                }
...
impl <T> SwigDeref for Vec<T> {
    ...
    =
    [T];
    fn swig_deref(&self) -> &Self::Target { &*self }
}
impl SwigDeref for JavaIntArray {
    ...
    =
    [i32];
    fn swig_deref(&self) -> &Self::Target { self.to_slice() }
}
...
impl SwigFrom<isize> for jint {
    fn swig_from(x: isize, _: *mut JNIEnv) -> Self { x as jint }
}
#[cfg(target_pointer_width = "64")]
impl SwigFrom<isize> for jlong {
    fn swig_from(x: isize, _: *mut JNIEnv) -> Self { x as jlong }
}
#[cfg(target_pointer_width = "32")]
impl SwigFrom<usize> for jlong {
    fn swig_from(x: usize, _: *mut JNIEnv) -> Self { x as jlong }
}
#[cfg(target_pointer_width = "64")]
impl SwigFrom<usize> for jlong {
